...
void TFT_eSPI::readRect(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t *data)
{
  ...
  while (dh--) {
    ...
    while (lw--) {
      ...
      //new 2022 huebner
      if (alpha<0xFF){
       //color=alphaBlend(alpha,alphaColor,color);
      } 
...
boolean transpSet=false;
  ...
  while (dh--) {
    ...
    while (len--) {
     transpSet=((setTransp) && (transp == *ptr)); 
     //transpSet=(transp == *ptr); 
...
/***************************************************************************************
** Function name:           pushImageAlpha with dataAlpha changed huebner 2022
** Description:             plot 16 bit sprite or image with 1 colour being transparent
***************************************************************************************/
void TFT_eSPI::pushImageAlpha(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t *data, uint8_t *dataAlpha,uint16_t transp)
{
  PI_CLIP;
  ...
  if (!_swapBytes) transp = transp >> 8 | transp << 8;
  //
  if (data!=NULL) data += dx + dy * w;
  //
  //Serial.print("pushImageAlpha=");
  //Serial.println(setTransp);
  //
  while (dh--)
  {
    //new huebner 2022
    //readRect(x,y,dw,1,(uint16_t*)lineBuf);
    int32_t len = dw;
    uint16_t* ptr      = data;     
    uint8_t*  ptrAlpha = dataAlpha;
    int32_t px = x, sx = x;
    bool move = true;
    uint16_t np = 0;
    //setTransp=false;
    ...
    while (len--)
    {
      if (((ptr!=NULL) && (transp != *ptr)) || (!setTransp))
      {
        lColor=lineBuf[np];
          if (ptr!=NULL) aColor=*ptr;else aColor=transp;
          if (move) { move = false; sx = px; }
          //
          if (ptrAlpha!=NULL) ap=*ptrAlpha; else ap=alpha;           
          // 
          if (ap<0xFF){
            aColor=aColor<<8 | aColor>>8;
            lColor=lColor<<8 | lColor>>8;
            aColor=alphaBlend(ap,aColor,lColor);
            aColor=aColor<<8 | aColor>>8;
           } 
        lineBuf[np]=aColor;
          //
          if (setScreenShot) *ptr=aColor >> 8 | aColor << 8;
...
void TFT_eSPI::setWindow(int32_t xs, int32_t ys, int32_t xe, int32_t ye) {
  ...
  SPI1W0 =
      (xs >> 8) | (uint16_t)(xs << 8) | ((uint8_t)(xe >> 8) << 16 | (xe << 24));
  SPI1CMD |= SPIBUSY;
  while (SPI1CMD & SPIBUSY) {
  }
  ...
  // Load the two coords as a 32 bit value and shift in one go
  SPI1W0 =
      (ys >> 8) | (uint16_t)(ys << 8) | ((uint8_t)(ye >> 8) << 16 | (ye << 24));
  SPI1CMD |= SPIBUSY;
  while (SPI1CMD & SPIBUSY) {
  }
...
#if defined( \
    RPI_ILI9486_DRIVER)  // This is for the RPi display that needs 16 bits per byte
...
void TFT_eSPI::drawPixel(int32_t x, int32_t y, uint32_t color,uint8_t alpha) {
  if (_vpOoB) return;
...
** Description:             draw Pixels data
***************************************************************************************/

void TFT_eSPI::pushPixels(uint16_t *data, uint32_t len){
 pushColors((uint16_t*) data, len,_swapBytes);
}
...
void TFT_eSPI::setFreeFont(const GFXfont *f) {
  ...
  if (pgm_read_word(&gfxFont->range_num) != 0) {
    ...
    for (uint16_t i = 0; i < pgm_read_word(&gfxFont->range_num); i++) {
      numChars +=
          pgm_read_word(&range_pst[i].end) - pgm_read_word(&range_pst[i].start);
    }
  } else {
...
void writeBlock(uint16_t color, uint32_t repeat) {
  ...
  if (repeat > 19) {
    SET_PERI_REG_BITS(SPI_MOSI_DLEN_REG(SPI_PORT), SPI_USR_MOSI_DBITLEN, 479,
...
void TFT_eSPI::loadFont(String fontName, bool flash) {
  ...
  */
  ...
  if (fontName == "") {
    //Serial.println("STREAM-POINTER");
    fs_font = false;
  }else { 
    fontPtr = nullptr; fs_font = true;
  }
  //
  if (fs_font) {
...
#endif 
  //
  //Serial.println("0.Font file " + fontName);
  ...
  //Serial.println("1.Font file " + fontName);
  fontLoaded = true;
...
void TFT_eSPI::loadMetrics(void)
{
 ...
 uint16_t gNum = 0;
 //
 //Serial.println("gFont.gCount=" + String(gFont.gCount));
  ...
  while (gNum < gFont.gCount)
  {
    ...
    readInt32(); // ignored
    //
    //Serial.print(String(gNum) + "\t");
    //Serial.print("Unicode = 0x"); Serial.println(gUnicode[gNum], HEX);
  ...
  //Serial.println("loadMetrics=" + String(gFont.gCount)); 
}
