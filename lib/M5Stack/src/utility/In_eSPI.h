...
#define set_mask(C) \
  xset_mask         \
      [C]  // 63fps Sprite rendering test 33% faster, graphicstest only 1.8% faster than shifting in real time
...
class TFT_eSPI : public Print {
 public:
  ...
  void init(uint8_t tc = TAB_COLOUR),
      begin(
          uint8_t tc =
              TAB_COLOUR);  // Same - begin included for backwards compatibility

  // These are virtual so the TFT_eSprite class can override them with sprite specific functions
  virtual void drawPixel(int32_t x, int32_t y, uint32_t color,uint8_t alpha=0xFF),
  ...
  void setWindow(int32_t xs, int32_t ys, int32_t xe, int32_t ye),
      ...
      pushPixels(uint16_t *data, uint32_t len),
  ...
  void pushImageAlpha(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t *data, uint8_t* dataAlpha, uint16_t transparent);
...
// The TFT_eSprite class inherits the following functions (not all are useful to Sprite class
 ...
 //new huebner
  ...
  //new huebner
  uint8_t  alpha=0xFF; 
  uint16_t alphaColor=0xFFFF; 

  boolean setScreenShot=false;
  boolean setTransp=false;
  //new r.huebner
  //
 private:
 ...
 protected:
  ...
  bool textwrapX,
      textwrapY;  // If set, 'wrap' text at right and optionally bottom edge of display
